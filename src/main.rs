...
impl TodoItem {
    fn new(name: String) -> TodoItem {
        return TodoItem { name: name, completed: ' ' };
    }
}
...
fn main() {
    ...
    let command = match arguments[1].as_str() {
        ...
        "done" => Command::Done(arguments[2].parse().expect("error converting to number")),
        "remove" => Command::Remove(arguments[2].parse().expect("error parsing for remove")),
        _ => panic!("You must provide an accepted command get|add|done|remove"),
    };
